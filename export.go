...
func (md *Mysqldump) Export() (outFile string, err error) {
	...
	// 获取建库语句
	createSQL, err := md.GetCreateDbSQL(md.cfg.DbCfg.DbName)
	if err != nil {
		return "", err
	}
	// 获取数据库字符集
	...
	if md.cfg.IsCreateDB == true {
		createSQL = fmt.Sprintf("DROP DATABASE IF EXISTS `%s`;\nCREATE DATABASE `%s`;", md.cfg.DbCfg.DbName+"_test", md.cfg.DbCfg.DbName+"_test")
	} else {
		createSQL = ""
	}
	...
	useSQL := ""
	if md.cfg.IsCreateDB == true {
		useSQL = fmt.Sprintf("USE `%s`;", md.cfg.DbCfg.DbName+"_test")
	} else {
		useSQL = fmt.Sprintf("USE `%s`;", md.cfg.DbCfg.DbName)
	}
...
func (md *Mysqldump) GetCreateDbSQL(name string) (string, error) {
	createSQLs := make([]*CreateDb, 0)
	err := md.conn.SQL(fmt.Sprintf("SHOW CREATE DATABASE %s", name)).Find(&createSQLs)
	if err != nil {
		return "", err
	}
	if len(createSQLs) == 0 {
...
func (md *Mysqldump) ShowDatabases() ([]string, error) {
	databases := []string{}
	err := md.conn.SQL("show databases").Find(&databases)
	if err != nil {
